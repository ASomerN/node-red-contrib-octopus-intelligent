    ...
    account(accountNumber: $account) {
        electricityAgreements {
            ...
            tariff {
                ...
                ... on DayNightTariff { productCode standingCharge dayRate nightRate }
                ... on FourRateEvTariff { productCode standingCharge dayRate nightRate evDevicePeakRate evDeviceOffPeakRate }
...
function parseRatesResponse(data) {
    ...
    const fieldsFor = (prefix, agreements) => {
        ...
        // Kraken Float scalars can serialise as strings — coerce defensively
        return {
            [`${prefix}_unit_rate`]: tariff.unitRate != null ? parseFloat(tariff.unitRate) : null,
            [`${prefix}_standing_charge`]: tariff.standingCharge != null ? parseFloat(tariff.standingCharge) : null,
            ...
            // v1.5 rate-band fields — null on tariffs that don't expose them
            [`${prefix}_day_rate`]: tariff.dayRate != null ? parseFloat(tariff.dayRate) : null,
            [`${prefix}_night_rate`]: tariff.nightRate != null ? parseFloat(tariff.nightRate) : null,
            [`${prefix}_ev_peak_rate`]: tariff.evDevicePeakRate != null ? parseFloat(tariff.evDevicePeakRate) : null,
            [`${prefix}_ev_off_peak_rate`]: tariff.evDeviceOffPeakRate != null ? parseFloat(tariff.evDeviceOffPeakRate) : null,
        };
    };
...
const defaultData = {
    ...
    electricity_day_rate: null,
    electricity_night_rate: null,
    electricity_ev_peak_rate: null,
    electricity_ev_off_peak_rate: null,
    ...
    electricity_export_day_rate: null,
    electricity_export_night_rate: null,
    electricity_export_ev_peak_rate: null,
    electricity_export_ev_off_peak_rate: null,
    electricity_rates_error: null,
